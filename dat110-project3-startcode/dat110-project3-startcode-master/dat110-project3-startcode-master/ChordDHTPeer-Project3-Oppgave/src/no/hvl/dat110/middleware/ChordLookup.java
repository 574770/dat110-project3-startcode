...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		NodeInterface k =  node.findSuccessor(key);
		...
		if(Util.computeLogic(key,node.getNodeID().add(BigInteger.valueOf(1)),s.getNodeID())==true) {
			return s;
		}else {
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		for(int i = fTable.size()-1;i>0;i--) {
			NodeInterface n = fTable.get(i);
			if(Util.computeLogic(n.getNodeID(),node.getNodeID().add(BigInteger.valueOf(1)),key)==true) {
				return n;
			}
