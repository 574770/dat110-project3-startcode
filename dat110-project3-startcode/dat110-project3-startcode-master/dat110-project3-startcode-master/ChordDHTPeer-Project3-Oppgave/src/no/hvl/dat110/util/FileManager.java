...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		String fName;
		for(int i = 0;i<Util.numReplicas-1;i++) {
			fName=filename +i;
			replicafiles[replicafiles.length-1]=Hash.hashOf(fName);
		}
    ...
    public int distributeReplicastoPeers() throws RemoteException {
    	...
    	for(int i = 0; i<replicafiles.length-1;i++) {
    		BigInteger replica = replicafiles[i];
    		NodeInterface s = chordnode.findSuccessor(replica);
    		s.addKey(replica);
    		
    		if(i==index) {
    			s.saveFileContent(filename,hash, bytesOfFile, true);
    		}
    		else {
    			s.saveFileContent(filename,hash, bytesOfFile, false);
    		}
    		counter++;
    	}
	...
	public NodeInterface findPrimaryOfItem() {
		...
		try {
			...
			NodeInterface[] tab = (NodeInterface[]) nodes.toArray();
			for(int i = 0; i<nodes.size()-1;i++) {
				if(tab[i].getFilesMetadata(hash).isPrimaryServer()==true) {
					return tab[i];
				}
			}
